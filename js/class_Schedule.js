    ...
    for (var event of this.auto_schedule) {
      ...
      if (event.days == 1) {
        ...
        for (; i < times.length; i++) {
          if (event.specified_time[1] <= times[i][0]) {
            // 各タスクの終了時間が固定時間の開始時間より速いなら
            break;
          }
          if (
            (times[i][0] >= event.specified_time[0] &&
              times[i][0] < event.specified_time[1]) || // スタートをまたいでいないか?
            (times[i][1] > event.specified_time[0] &&
              times[i][1] <= event.specified_time[1]) || // エンドまたいでいないか?
            (times[i][0] <= event.specified_time[0] &&
              times[i][1] >= event.specified_time[1])
          ) {
            // 元からある予定の間にすっぽり埋もれていないか?
            event.specified_time[0] = times[i][1] + 10 * 60 * 1000; // 10分の休憩を持たせておく.
            event.specified_time[1] =
              event.specified_time[0] + event.required_time;
          }
        }
        if (event.specified_time[1] > event.deadline) {
          // 各タスクの終了時間が締め切りを過ぎていたら
          console.log(
            "この予定「" +
              event.name +
              "」の\n終了時間:" +
              new Date(event.specified_time[1]).toString() +
              "は\n締切時間:" +
              new Date(event.deadline).toString() +
              "を過ぎてしまいます."
          );
        ...
        for (var i = 0; i < event.task_children.length; i++) {
          ...
          console.log(
            i +
              1 +
              "個目の子タスクの\n開始時間:" +
              new Date(event.task_children[i].specified_time[0]).toString() +
              "\n終了時間:" +
              new Date(event.task_children[i].specified_time[1]).toString()
          );
        }
  ...
  returnAllTasks() {
    ...
    while (i < this.on_time.length || j < this.auto_schedule.length) {
      if (i >= this.on_time.length) {
        ...
        if (
          this.on_time[i].specified_time[0] >
          this.auto_schedule[j].specified_time[0]
        ) {
          all_tasks.push(this.auto_schedule[j]);
          j++;
        } else {
    ...
    for (const events of all_tasks) {
      console.log(
        "タスク名:" +
          ...
          new Date(events.specified_time[1]).toString() +
          "\n締切時間:" +
          new Date(events.deadline).toString()
      );
    }
    return all_tasks;
  }
